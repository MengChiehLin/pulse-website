// English Terms of Service Content
const termsen = {
    title: 'Terms of Service',
    lastUpdated: 'Last Updated: January 13, 2025',
    backToHome: '← Back to Home',
    pageTitle: 'Terms of Service - PULSE',
    mainContent: `
        <h1>Terms of Service</h1>
        <p><strong>Last Updated: January 13, 2025</strong></p>
        
        <!-- Table of Contents -->
        <div class="toc">
            <h3>Table of Contents</h3>
            <ul>
                <li><a href="#acceptance">1. Acceptance of Terms</a></li>
                <li><a href="#service-description">2. Service Description</a></li>
                <li><a href="#account-terms">3. Account Terms</a></li>
                <li><a href="#subscription-terms">4. Subscription and Payment Terms</a></li>
                <li><a href="#acceptable-use">5. Acceptable Use</a></li>
                <li><a href="#prohibited-use">6. Prohibited Use</a></li>
                <li><a href="#intellectual-property">7. Intellectual Property</a></li>
                <li><a href="#content-disclaimer">8. Content Disclaimer</a></li>
                <li><a href="#investment-disclaimer">9. Investment Advice Disclaimer</a></li>
                <li><a href="#privacy">10. Privacy Policy</a></li>
                <li><a href="#liability">11. Limitation of Liability</a></li>
                <li><a href="#indemnification">12. Indemnification</a></li>
                <li><a href="#termination">13. Service Termination</a></li>
                <li><a href="#dispute-resolution">14. Dispute Resolution</a></li>
                <li><a href="#changes">15. Changes to Terms</a></li>
                <li><a href="#contact">16. Contact Information</a></li>
            </ul>
        </div>
        
        <h2 id="acceptance">1. Acceptance of Terms</h2>
        <p>Welcome to PULSE! These Terms of Service ("Terms") constitute a legally binding agreement between you and PULSE. By using our service, you agree to comply with these terms. If you do not agree to these terms, please do not use our service.</p>
        
        <h2 id="service-description">2. Service Description</h2>
        <p>PULSE is an AI-powered news alert application that provides the following services:</p>
        <ul>
            <li>Real-time international news monitoring and filtering</li>
            <li>AI intelligent translation and summarization services</li>
            <li>Important news push notifications</li>
            <li>Personalized news reading experience</li>
            <li>Multi-language support (Traditional Chinese, Simplified Chinese, English)</li>
        </ul>
        
        <h2 id="account-terms">3. Account Terms</h2>
        
        <h3>3.1 Age Requirements</h3>
        <p>You must be at least 13 years old (16 in the EU) to use PULSE. Users under the required age may not register or use this service.</p>
        
        <h3>3.2 Account Registration</h3>
        <ul>
            <li>You need to provide a valid email address to register an account</li>
            <li>You are responsible for maintaining account security and keeping login information confidential</li>
            <li>Each user may only register one account</li>
            <li>You are responsible for all activities under your account</li>
        </ul>
        
        <h3>3.3 Account Deletion</h3>
        <div class="warning-box">
            <p><strong>IMPORTANT:</strong> Account deletion is permanent and cannot be recovered. All personal data will be immediately deleted with no grace period.</p>
        </div>
        <ul>
            <li>You can request account deletion at any time</li>
            <li>Please send an email to support@pulse-news.app</li>
            <li>Deletion processing will be completed within 24 hours</li>
            <li>No data or subscriptions can be recovered after deletion</li>
        </ul>
        
        <h2 id="subscription-terms">4. Subscription and Payment Terms</h2>
        
        <h3>4.1 Service Plans</h3>
        <div class="highlight-box">
            <p><strong>Free Plan:</strong></p>
            <ul>
                <li>Unlimited browsing of non-critical international news in Chinese</li>
                <li>Basic AI translation and summarization features</li>
                <li>Permanent free use</li>
            </ul>
            
            <p><strong>Monthly Membership ($4.99/month):</strong></p>
            <ul>
                <li>AI-selected important news alerts</li>
                <li>Complete AI intelligent summarization and translation</li>
                <li>Real-time push notifications</li>
                <li>Multi-device sync support</li>
                <li>24/7 news monitoring</li>
            </ul>
        </div>
        
        <h3>4.2 Free Trial</h3>
        <ul>
            <li>First-time registered users can try all monthly membership features for free</li>
            <li>No payment information required during trial</li>
            <li>Automatically converts to free membership after trial ends</li>
            <li>Users can upgrade to paid plan at any time</li>
        </ul>
        
        <h3>4.3 Payment Processing</h3>
        <ul>
            <li>All payments are securely processed through Stripe</li>
            <li>We do not store your credit card information</li>
            <li>Subscriptions will auto-renew unless you cancel</li>
            <li>Prices may vary by region</li>
        </ul>
        
        <h3>4.4 Cancellation and Refunds</h3>
        <ul>
            <li>You can cancel your subscription at any time</li>
            <li>Service will continue until the end of the current billing period</li>
            <li>For refund requests, please contact support@pulse-news.app</li>
            <li>Refunds will be processed on a case-by-case basis</li>
        </ul>
        
        <h2 id="acceptable-use">5. Acceptable Use</h2>
        <p>You agree to:</p>
        <ul>
            <li>Use PULSE only for lawful purposes</li>
            <li>Comply with all applicable laws and regulations</li>
            <li>Respect the rights of other users and third parties</li>
            <li>Use the service only for personal, non-commercial purposes</li>
            <li>Not attempt to circumvent technical limitations or security measures</li>
        </ul>
        
        <h2 id="prohibited-use">6. Prohibited Use</h2>
        <p>You may not:</p>
        <ul>
            <li>Use automated tools or bots to access the service</li>
            <li>Attempt unauthorized access to systems or data</li>
            <li>Interfere with or disrupt the normal operation of the service</li>
            <li>Distribute malware or conduct cyber attacks</li>
            <li>Violate any applicable laws or regulations</li>
            <li>Infringe on others' intellectual property</li>
            <li>Engage in any behavior that could damage the service's reputation</li>
        </ul>
        
        <h2 id="intellectual-property">7. Intellectual Property</h2>
        
        <h3>7.1 Our Rights</h3>
        <ul>
            <li>PULSE application, trademarks, and designs belong to us</li>
            <li>Editorial rights to AI summaries and translations belong to us</li>
            <li>All technical implementations of the service are protected by intellectual property</li>
        </ul>
        
        <h3>7.2 Third-Party Content</h3>
        <ul>
            <li>Original news content belongs to respective publishers</li>
            <li>We only provide summarization services within fair use</li>
            <li>Users may not redistribute or commercially use third-party content</li>
        </ul>
        
        <h2 id="content-disclaimer">8. Content Disclaimer</h2>
        
        <div class="warning-box">
            <p><strong>IMPORTANT DISCLAIMER:</strong> News content and AI summaries provided by PULSE are for reference only and do not constitute any form of advice.</p>
        </div>
        
        <ul>
            <li>News content comes from third-party providers (MediaStack, RapidAPI, etc.)</li>
            <li>AI summaries are generated by OpenAI technology and may contain errors</li>
            <li>We strive to ensure information accuracy but do not guarantee completeness or timeliness</li>
            <li>Users should independently verify the accuracy of important information</li>
            <li>We are not responsible for the accuracy of third-party content</li>
        </ul>
        
        <h2 id="investment-disclaimer">9. Investment Advice Disclaimer</h2>
        
        <div class="warning-box">
            <p><strong>INVESTMENT RISK WARNING:</strong> PULSE does not provide investment advice, and all investment decisions should be based on your own research and judgment.</p>
        </div>
        
        <ul>
            <li>Information provided by PULSE is for general reference only</li>
            <li>Does not constitute investment, legal, or financial advice</li>
            <li>Investing involves risks and may result in loss of principal</li>
            <li>Please consult professionals before making investment decisions</li>
            <li>We are not responsible for investment decisions based on our information</li>
        </ul>
        
        <h2 id="privacy">10. Privacy Policy</h2>
        <p>Your privacy is important to us. Please refer to our <a href="privacy.html" style="color: var(--gold);">Privacy Policy</a> to understand how we collect, use, and protect your personal information. By using our service, you agree to our privacy policy.</p>
        
        <h2 id="liability">11. Limitation of Liability</h2>
        
        <h3>11.1 Service Basis</h3>
        <p>PULSE is provided on an "as is" and "available" basis, without any express or implied warranties, including but not limited to:</p>
        <ul>
            <li>Merchantability warranty</li>
            <li>Fitness for particular purpose warranty</li>
            <li>Non-infringement warranty</li>
            <li>Continuous availability warranty</li>
        </ul>
        
        <h3>11.2 Limitation of Damages</h3>
        <p>To the maximum extent permitted by law, we are not liable for the following damages:</p>
        <ul>
            <li>Indirect, incidental, or punitive damages</li>
            <li>Loss of profits or data</li>
            <li>Business interruption or investment losses</li>
            <li>Any damages caused by third-party content</li>
        </ul>
        
        <h3>11.3 Liability Cap</h3>
        <p>Under any circumstances, our total liability shall not exceed the fees you paid to us in the past 12 months, or $100 USD (whichever is higher).</p>
        
        <h2 id="indemnification">12. Indemnification</h2>
        <p>You agree to indemnify and hold us harmless from any claims, losses, or expenses arising from your violation of these terms or misuse of the service.</p>
        
        <h2 id="termination">13. Service Termination</h2>
        
        <h3>13.1 User Termination</h3>
        <ul>
            <li>You can stop using the service at any time</li>
            <li>Account deletion requests will be processed immediately</li>
            <li>Paid subscriptions will continue until the end of the current billing period</li>
        </ul>
        
        <h3>13.2 Our Termination Rights</h3>
        <p>We may suspend or terminate your account in the following situations:</p>
        <ul>
            <li>Violation of these terms of service</li>
            <li>Engaging in illegal or harmful activities</li>
            <li>Long-term non-use of account (over 2 years)</li>
            <li>Technical or security considerations</li>
        </ul>
        
        <h2 id="dispute-resolution">14. Dispute Resolution</h2>
        
        <h3>14.1 Negotiated Resolution</h3>
        <p>If disputes arise, we encourage resolution through friendly negotiation. Please contact support@pulse-news.app first.</p>
        
        <h3>14.2 Applicable Law</h3>
        <p>These terms are governed by the laws of the Republic of China, without regard to conflict of law principles.</p>
        
        <h3>14.3 Jurisdiction</h3>
        <p>Any legal disputes will be under the exclusive jurisdiction of the Taipei District Court in Taiwan.</p>
        
        <h2 id="changes">15. Changes to Terms</h2>
        <ul>
            <li>We may update these terms of service from time to time</li>
            <li>Major changes will be notified via email or in-app notification</li>
            <li>Changes will take effect 30 days after announcement</li>
            <li>Continued use of the service indicates acceptance of changes</li>
            <li>If you do not agree to changes, please stop using the service</li>
        </ul>
        
        <h2 id="contact">16. Contact Information</h2>
        <p>If you have any questions about these terms of service, please contact us through the following methods:</p>
        
        <p><strong>Email:</strong> support@pulse-news.app<br>
        <strong>Website:</strong> https://pulse-news.app<br>
        <strong>Response Time:</strong> We will respond to your inquiry within 30 days</p>
        
        <div class="last-updated">
            <p><em>These terms of service were last updated on January 13, 2025</em></p>
        </div>
    `
};

// Make sure the variable is available globally
window.termsen = termsen;

// Export for use in other files
if (typeof module !== 'undefined' && module.exports) {
    module.exports = termsen;
}